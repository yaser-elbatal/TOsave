...
import { List_Reports_Statistics_1 } from '../../../../../';
      ...
      unique(arrayOfAll, "branchOryearOrMonth")
        .map(u => {
          arrayOfAll.map(ar => {
            ...
            if (ar.monthely_branch.name_ar != this.state.selectedBranchName) return

            if (ar.branchOryearOrMonth == u.branchOryearOrMonth &&
              ar.monthely_branch.name_ar == branchName) {
              u[`s${ar.status}`] += 1
              return ar
            }
          })
                ...
                ({ loading, error, data }) => {
                  ...
                  if (data.monthely_report.length) {
                        ...
                        branchOryearOrMonth:
                          radioSelected == "year" ? rep.year :
                            radioSelected == "month" ? `${rep.month}/${rep.year}` :
                              radioSelected == "branches" ? rep.monthely_branch.name_ar :
                    ...
                    let
                      ...
                      branchesName =
                        getUniqueArray(allReports.map(arn => arn.monthely_branch.name_ar))
                          ...
                          {(this.state.radioSelected != "branch") &&
                            <Row className="text-center">
                              <Col sm={12} md className="mb-sm-2 mb-0">
                                ...
                                <strong>{`${s0.reduce((ac, el) => ac += el, 0)}`} ({`${parseInt(s0.reduce((ac, el) => ac += el, 0) * 100 / allS.reduce((ac, el) => ac += el, 0))}`}%)</strong>
                              ...
                              <Col sm={12} md className="mb-sm-2 mb-0">
                                ...
                                <strong>{`${s1.reduce((ac, el) => ac += el, 0)}`} ({`${parseInt(s1.reduce((ac, el) => ac += el, 0) * 100 / allS.reduce((ac, el) => ac += el, 0))}`}%)</strong>
                              ...
                              <Col sm={12} md className="mb-sm-2 mb-0">
                                ...
                                <strong>{`${s2.reduce((ac, el) => ac += el, 0)}`} ({`${parseInt(s2.reduce((ac, el) => ac += el, 0) * 100 / allS.reduce((ac, el) => ac += el, 0))}`}%)</strong>
