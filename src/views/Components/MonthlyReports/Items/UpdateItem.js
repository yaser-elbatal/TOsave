...
import { List_Items, Update_Item } from '../../../../services/queries/MonthlyReports/Items'
...
export default class UpdateList extends Component {
    ...
    updateHandle = (updateitem) => {
        let item = this.state.item
        let itemNewData = this.state.updatedData;
        item = { ...item, ...itemNewData };
        item.images_number = parseInt(item.images_number)
        console.log(item);
